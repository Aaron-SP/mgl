...
bool test_vec2()
{
    ...
    min::stack_vector<size_t, min::vec2<double>::sub_size()> keys;
    ...
    min::vec2<double>::subdivide_overlap(sub_over, one, two, three);
    ...
    if (!out)
    {
        throw std::runtime_error("Failed vec2 subdivide_overlap");
    }
