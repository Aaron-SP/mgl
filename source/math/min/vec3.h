...
*/
#ifndef _MGL_VECTOR3_MGL_
#define _MGL_VECTOR3_MGL_
...
class vec3
{
  ...
  public:
    ...
    inline static auto grid_overlap(const vec3<T> &min, const vec3<T> &extent, const size_t scale, const vec3<T> &b_min, const vec3<T> &b_max)
    {
        min::stack_vector<size_t, vec3<T>::over_size()> out;
    ...
    inline static auto subdivide(C &out, const vec3<T> &min, const vec3<T> &max)
    {
        ...
        // Octant 1
        const vec3<T> min1 = vec3<T>(min.x(), min.y(), c.z());
        const vec3<T> max1 = vec3<T>(c.x(), c.y(), max.z());

        // Octant 2
        const vec3<T> min2 = vec3<T>(min.x(), c.y(), min.z());
        const vec3<T> max2 = vec3<T>(c.x(), max.y(), c.z());

        // Octant 3
        const vec3<T> min3 = vec3<T>(min.x(), c.y(), c.z());
        const vec3<T> max3 = vec3<T>(c.x(), max.y(), max.z());

        // Octant 4
        const vec3<T> min4 = vec3<T>(c.x(), min.y(), min.z());
        const vec3<T> max4 = vec3<T>(max.x(), c.y(), c.z());

        // Octant 5
        const vec3<T> min5 = vec3<T>(c.x(), min.y(), c.z());
        const vec3<T> max5 = vec3<T>(max.x(), c.y(), max.z());

        // Octant 6
        const vec3<T> min6 = vec3<T>(c.x(), c.y(), min.z());
        const vec3<T> max6 = vec3<T>(max.x(), max.y(), c.z());
