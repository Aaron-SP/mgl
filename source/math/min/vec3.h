...
class vec3
{
  ...
  public:
    ...
    inline static size_t grid_key(const std::tuple<size_t, size_t, size_t> &index, const size_t scale, const vec3<T> &point)
    {
        // Get the row / col of cell
        const size_t col = std::get<0>(index);
        const size_t row = std::get<1>(index);
    ...
    static inline std::pair<vec3<T>, T> project_sat_penetration(
        const coord_sys<T, vec3> &axis1, const vec3<T> &center1, const vec3<T> &extent1,
        const coord_sys<T, vec3> &axis2, const vec3<T> &center2, const vec3<T> &extent2, const T tolerance)
    {
        ...
        const vec3<T> &center2, const vec3<T> &extent2, const T tolerance)
    {
        ...
        const vec3<T> d = center1 - center2;
        const vec3<T> t = vec3<T>(d).abs();
        ...
        const vec3<T> dL = (extent1 + extent2 + tolerance) - t;
