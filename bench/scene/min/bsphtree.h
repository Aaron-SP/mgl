...
*/
#ifndef __BENCHSPHERETREE__
#define __BENCHSPHERETREE__
...
double bench_sphere_aabb_tree(size_t N)
{
    // Running sphere_aabbox_tree test
    std::cout << "sphere_aabb_tree: Starting benchmark with " << N << " tree insertions" << std::endl;
    ...
    min::tree<T, uint16_t, uint32_t, vec, min::sphere, min::aabbox> t(world);
    ...
    // Report tree depth
    std::cout << "sphere_aabb_tree: Tree depth is: " << depth << std::endl;

    // Report collisions found
    std::cout << "sphere_aabb_tree: Collisions found: " << collisions.size() << std::endl;

    if (collisions.size() != col_count)
    {
        std::cout << "sphere_aabb_tree: Failed benchmark, wrong collision count." << std::endl;
    }

    // Print the execution time
    double out = std::chrono::duration<double, std::milli>(dtime).count();
    std::cout << "sphere_aabb_tree: tests completed in: " << out << " ms" << std::endl;
...
template <typename T, size_t col_count, template <typename> class vec>
double bench_sphere_sphere_tree(size_t N)
{
    // Running sphere_sphere_tree test
    std::cout << "sphere_sphere_tree: Starting benchmark with " << N << " tree insertions" << std::endl;
    ...
    vec<T> maxW = vec<T>().set_all(high);
    min::sphere<T, vec> world(minW, maxW);
    min::tree<T, uint16_t, uint32_t, vec, min::sphere, min::sphere> t(world);
    ...
    // Report tree depth
    std::cout << "sphere_sphere_tree: Tree depth is: " << depth << std::endl;

    // Report collisions found
    std::cout << "sphere_sphere_tree: Collisions found: " << collisions.size() << std::endl;

    if (collisions.size() != col_count)
    {
        std::cout << "sphere_sphere_tree: Failed benchmark, wrong collision count." << std::endl;
    }

    // Print the execution time
    double out = std::chrono::duration<double, std::milli>(dtime).count();
    std::cout << "sphere_sphere_tree: tests completed in: " << out << " ms" << std::endl;
