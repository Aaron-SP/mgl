...
*/
#ifndef __BENCHAABBGRID__
#define __BENCHAABBGRID__
...
double bench_aabb_aabb_grid(size_t N)
{
    // Running aabb_aabb_grid test
    std::cout << "aabb_aabb_grid: Starting benchmark with " << N << " grid insertions" << std::endl;
    ...
    min::grid<T, uint16_t, uint32_t, vec, min::aabbox, min::aabbox> g(world);
    ...
    // Report grid scale
    std::cout << "aabb_aabb_grid: grid scale is: " << scale << std::endl;

    // Report collisions found
    std::cout << "aabb_aabb_grid: Collisions found: " << collisions.size() << std::endl;

    if (collisions.size() != col_count)
    {
        std::cout << "aabb_aabb_grid: Failed benchmark, wrong collision count" << std::endl;
    }

    // Print the execution time
    double out = std::chrono::duration<double, std::milli>(dtime).count();
    std::cout << "aabb_aabb_grid: tests completed in: " << out << " ms" << std::endl;
...
template <typename T, size_t col_count, template <typename> class vec>
double bench_aabb_sphere_grid(size_t N)
{
    // Running aabb_sphere_grid test
    std::cout << "aabb_sphere_grid: Starting benchmark with " << N << " grid insertions" << std::endl;
    ...
    vec<T> maxW = vec<T>().set_all(high);
    min::aabbox<T, vec> world(minW, maxW);
    min::grid<T, uint16_t, uint32_t, vec, min::aabbox, min::sphere> g(world);
    ...
    // Report grid scale
    std::cout << "aabb_sphere_grid: grid scale is: " << scale << std::endl;

    // Report collisions found
    std::cout << "aabb_sphere_grid: Collisions found: " << collisions.size() << std::endl;

    if (collisions.size() != col_count)
    {
        std::cout << "aabb_sphere_grid: Failed benchmark, wrong collision count" << std::endl;
    }

    // Print the execution time
    double out = std::chrono::duration<double, std::milli>(dtime).count();
    std::cout << "aabb_sphere_grid: tests completed in: " << out << " ms" << std::endl;
